...
namespace Text_Based_RPG_Shell_Knight
{
	...
	class Inventory
	{
		...
		public void IncreaseStock(ITEM ID)// FindItem Child
		{
			stockItems[(int)ID]++;
		}
...
*/
                if (cursorPos == 1) { UseHealthPot(player, item, hud); }
                else if (cursorPos == 2) { hud.DisplayText($"< empty >", false); }
                else if (cursorPos == 3) { hud.DisplayText($"< empty >", false); }
                else if (cursorPos == 4) { hud.DisplayText(" \"A Big Key, for Big Problems, I mean Big Doors\"", false); }
                else if (cursorPos == 5) { hud.DisplayText(" \"A Small Key, it looks generic, I bet it would fit most locks\"", false); }
                else if (cursorPos == 6) { hud.DisplayText($"< empty >", false); }
                else if (cursorPos == 7) { hud.DisplayText($"< empty >", false); }
                else if (cursorPos == 8) { hud.DisplayText($"< empty >", false); }
                else if (cursorPos == 9) { hud.DisplayText($"< empty >", false); }
                else if (cursorPos == 10) { hud.DisplayText($"< empty >", false); }
                else if (cursorPos == 11) { hud.DisplayText($"< empty >", false); }
                else if (cursorPos == 12) { hud.DisplayText($"< empty >", false); }
                else if (cursorPos == 13) { hud.DisplayText($"< empty >", false); }
                else if (cursorPos == 14) { hud.DisplayText($"< empty >", false); }
                else if (cursorPos == 15) { hud.DisplayText($"< empty >", false); }
                else if (cursorPos == 16) { hud.DisplayText($"< empty >", false); }
                else if (cursorPos == 17) { hud.DisplayText($"< empty >", false); }
                else if (cursorPos == 18) { hud.DisplayText($"< empty >", false); }
        ...
        /*public void UsePot(Player player, Item item, HUD hud, string effect)
		{
			for (int i = 0; i < Global.globalAccess.itemIDs.Count; i++)
			{
				if (effect == Global.globalAccess.ItemValues.effects[i])
				{
					...
					{
						hud.DisplayText($"< {player.Name()} {Global.ME} >", false);
					}
				}
			}
		}*/
        ...
        public void DecreaseStock(ITEM ID) // called when item is used
		{
			stockItems[(int)ID] -= 1;
		}
        #endregion
