    ...
    class TradeMenu
    {
        ...
        public void ChooseItem(Inventory inventory, Player player)
        {
            ...
            else if (vendorBeingTradedWith.type == Vendor.Type.POTIONEER)
            {
                ...
                Console.SetCursorPosition(10, 7); Console.WriteLine("2 - " + Global.ITEM_NAME(ITEM.POTSHELL) + " " + Global.ITEM_AVATAR(ITEM.POTSHELL));
        ...
        private void CompleteItemTransaction(string ID, Player player, Inventory inventory)
        {
            ...
            else
            {
                ...
                for (int i = 0; i < Global.globalAccess.itemIDs[ID]; i++)
                {
                    ...
                    if (ID == checkedID) { player.currentMoney = player.currentMoney - tradePrice; inventory.IncreaseStock(ITEM.POTHEAL); hud.DisplayText($"< " + player.Name() + " bought a " + Global.ITEM_NAME(ITEM.POTHEAL) + " >", false); }
